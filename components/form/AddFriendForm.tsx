...
import { Search, User2, UserPlus2 } from "lucide-react"
...
export default function AddFriendForm({ open, onClose }: AddFriendFormProps) {
  ...
  const { friends, setFriends, loading } = useFriendStore()
  ...
  const handleSendFriendRequest = async (id : string) => {
    try {
      ...
      if(res.status ===200 || res.status ===201) { 
        ...
        toast.error("Lời mới đã tồn tại hoặc có lỗi xảy ra.")
    }
  }
      ...
      <DialogContent className="sm:max-w-md rounded-2xl">
        ...
        <div className="space-y-4 mt-2 w-full">
          ...
          <div className="flex flex-col gap-1 w-full max-h-56 overflow-y-scroll scrollbar-none">
            ...
            {friends?.length > 0 ? friends?.map(f => (<div className="text-base font-medium text-gray-800 flex items-center justify-between gap-2 hover:bg-gray-100 transition-all border border-gray-200 rounded-md py-2 px-3">
             ...
             > Add <UserPlus2/> </Button>
              ...
              : <div key={null}> No data found.</div>
            }
